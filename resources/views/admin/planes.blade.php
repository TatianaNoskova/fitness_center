...
<h2 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-6">Planes Disponibles (Доступные тарифы)</h2>
...
{{-- ======================================================================= --}}
{{-- МОДАЛЬНОЕ ОКНО: СОЗДАНИЕ ТАРИФА --}}
{{-- ======================================================================= --}}
...
<div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 max-w-md w-full overflow-hidden flex flex-col antialiased">
        <div class="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
            <h3 class="text-base font-bold text-slate-900"><i class="bi bi-card-checklist text-rose-500 me-2"></i> Crear Nuevo Plan</h3>
            <a href="{{ route('planes.index') }}" class="text-slate-400 hover:text-slate-600 transition"><i class="bi bi-x-lg"></i></a>
        </div>

        <form action="{{ route('planes.store') }}" method="POST" class="p-6 space-y-4 text-left">
            @csrf
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Nombre del Plan</label>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Precio ($)</label>
                    <input type="number" name="precio" step="0.01" value="{{ old('precio') }}" placeholder="Ej. 150000" required 
                           class="w-full rounded-xl border-slate-200 text-sm focus:border-rose-500 focus:ring-rose-500/20">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Duración (Meses)</label>
                    <input type="number" name="duracion" value="{{ old('duracion', 30) }}" placeholder="Ej. 30" required 
                           class="w-full rounded-xl border-slate-200 text-sm focus:border-rose-500 focus:ring-rose-500/20">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="estado" required class="w-full rounded-xl border-slate-200 text-sm focus:border-rose-500 focus:ring-rose-500/20 shadow-sm">
...
{{-- ======================================================================= --}}
{{-- МОДАЛЬНОЕ ОКНО: РЕДАКТИРОВАНИЕ ТАРИФА --}}
{{-- ======================================================================= --}}
@if(request('edit_id'))
    ...
    <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-xl border border-slate-100 max-w-md w-full overflow-hidden flex flex-col antialiased">
            <div class="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <h3 class="text-base font-bold text-slate-900"><i class="bi bi-pencil text-amber-500 me-2"></i> Editar Plan</h3>
                <a href="{{ route('planes.index') }}" class="text-slate-400 hover:text-slate-600 transition"><i class="bi bi-x-lg"></i></a>
            </div>

            <form action="{{ route('planes.update', $planParaEditar->id) }}" method="POST" class="p-6 space-y-4 text-left">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre', $planParaEditar->nombre) }}" required 
                           class="w-full rounded-xl border-slate-200 text-sm focus:border-amber-500 focus:ring-amber-500/20">
                </div>
