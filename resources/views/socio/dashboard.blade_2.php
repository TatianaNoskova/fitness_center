        ...
        <div class="space-y-10">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex items-center space-x-4">
                    ...
                    {{-- Отображение категории в виджете шапки --}}
                    <div class="border-l border-slate-100 pl-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-10">
                    ...
                    {{-- ДИНАМИЧЕСКАЯ КАРТОЧКА СТРАТЕГИИ ТАРИФА --}}
                            ...
                            <label class="bg-gradient-to-br from-amber-50/40 to-white border border-amber-200 hover:border-amber-400 rounded-2xl p-5 flex items-start space-x-3 cursor-pointer shadow-sm transition-all has-[:checked]:border-amber-500 has-[:checked]:bg-amber-50/20">
                                ...
                                <div class="flex-1">
                                    <div class="flex justify-between items-baseline">
                                        ...
                                        <div class="text-right">
                                            @if(($combo->descuento ?? 0) > 0 && $combo->servicios && $combo->servicios->isNotEmpty())
                                                <span class="text-xs text-rose-500 line-through font-semibold block -mb-1">
                                                    ${{ number_format($combo->servicios->sum('precio'), 0) }}
                                                </span>
                                            @endif
                                            <span class="text-amber-600 font-black text-xl">
                                                ${{ number_format($combo->precio_calculado, 0) }}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    @if(($combo->descuento ?? 0) > 0 && $combo->servicios && $combo->servicios->isNotEmpty())
                                        <div class="mt-2">
                                            <span class="inline-flex items-center gap-1 bg-amber-100 border border-amber-200 text-amber-800 text-[10px] font-bold px-2 py-0.5 rounded-md shadow-2xs animate-pulse">
                                                <i class="bi bi-tags-fill text-amber-600"></i> Descuento del {{ $combo->descuento }}% aplicado
                                            </span>
                                        </div>
                                    @endif
                                </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100">
                        ...
                        <th class="p-4">Concepto / Detalle</th>
                ...
                <tbody class="text-sm font-medium text-slate-600">
                    @forelse($historialPagos as $pago)
                        <tr class="border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                            <td class="p-4 text-[#002d55] font-bold text-xs">{{ $pago->fecha }}</td>
                            <td class="p-4">
                                @if($pago->plan_id && $pago->plan)
                                    <div class="flex items-center gap-1.5">
                                        <span class="bg-blue-50 text-[#002d55] text-[10px] font-bold px-1.5 py-0.5 rounded border border-blue-200">PLAN</span>
                                        <span class="text-xs font-semibold text-[#002d55]">{{ $pago->plan->nombre }}</span>
                                    </div>
                                @elseif($pago->combo_id && $pago->combo)
                                    <div class="flex items-center gap-1.5">
                                        <span class="bg-amber-50 text-amber-700 text-[10px] font-bold px-1.5 py-0.5 rounded border border-amber-200">COMBO</span>
                                        <span class="text-xs font-bold text-slate-700">{{ $pago->combo->nombre }}</span>
                                        <span class="text-[10px] font-bold text-amber-600">(-{{ $pago->combo->descuento }}%)</span>
                                    </div>
                                @elseif($pago->servicio_id && $pago->servicio)
                                    <div class="flex items-center gap-1.5">
                                        <span class="bg-slate-100 text-slate-700 text-[10px] font-bold px-1.5 py-0.5 rounded border border-slate-200">EXTRA</span>
                                        <span class="text-xs font-medium text-slate-600">{{ $pago->servicio->nombre }}</span>
                                    </div>
                                @else
                                    <span class="text-xs text-slate-500">{{ $pago->detalles ?? 'Concepto general' }}</span>
                                @endif
                            </td>
                            <td class="p-4 text-slate-700 font-bold">${{ number_format($pago->monto, 2) }}</td>
                            <td class="p-4 text-xs text-slate-400">{{ $pago->metodo_pago }}</td>
                            <td class="p-4">
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    @if($pago->estado == 'PENDIENTE')
                                        <form action="{{ route('socio.pagar', $pago->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-bold px-2.5 py-1 rounded-xl uppercase tracking-wider flex items-center transition-colors shadow-2xs">
                                                <i class="bi bi-wallet2 mr-1"></i> Pagar
                                            </button>
                                        </form>

                                        @if(!$pago->plan_id)
                                            <form action="{{ route('socio.cancelarPago', $pago->id) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de que deseas quitar este servicio extra?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-rose-500 hover:text-rose-700 hover:bg-rose-50 p-1.5 rounded-lg transition-colors" title="Eliminar servicio contratado">
                                                    <i class="bi bi-trash3 text-base"></i>
                                                </button>
                                            </form>
                                        @endif
                                    @else
                                        <span class="text-emerald-600 flex items-center justify-center text-xs font-bold py-1">
                                            <i class="bi bi-check-circle-fill mr-1 text-emerald-500"></i> Listo
                                        </span>
                                    @endif
                                </div>
                            </td>
